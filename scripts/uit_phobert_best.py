...
def preprocess_data(df, tokenizer):
    ...
    for _, row in df.iterrows():
        # Baseline Format: [CLS] Context [SEP] Response [SEP]
        text = f"{str(row['context'])} [SEP] {str(row['response'])}"
        texts.append(text)
        if 'label' in row:
...
def run_baseline_inference(model_path):
    ...
    for _, row in tqdm(test_df.iterrows(), total=len(test_df), desc="Baseline Inference"):
        inputs = tokenizer(
            f"{str(row['context'])} [SEP] {str(row['response'])}", 
            return_tensors="pt", 
            truncation=True, 
            padding='max_length', 
            max_length=256
    ...
    print(f"\nPHOBERT BASELINE MACRO-F1: {macro_f1:.4f}")
