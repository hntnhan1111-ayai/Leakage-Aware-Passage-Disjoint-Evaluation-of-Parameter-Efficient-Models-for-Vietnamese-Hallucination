...
def create_prompt_no_jaccard(sample):
...
def train_hallucination_model():
    ...
    train_df['text'] = train_df.apply(lambda row: create_prompt_no_jaccard(row) + f" {row['label']}", axis=1)
...
def create_inference_prompt(sample, template_id):
    context = sample['context'] if pd.notna(sample['context']) else ""
    response = sample['response'] if pd.notna(sample['response']) else ""
...
def predict_single_with_confidence(model, tokenizer, context, response, template_id, seed):
    ...
    prompt = create_inference_prompt(sample, template_id)
    model.eval()
    inputs = tokenizer(prompt, return_tensors="pt", max_length=1024, truncation=True).to("cuda")
    set_seed(seed)
    with torch.no_grad():
        outputs = model.generate(**inputs, max_new_tokens=10, output_scores=True, return_dict_in_generate=True, do_sample=True, temperature=0.7, pad_token_id=tokenizer.eos_token_id)
...
def run_prediction_pipeline():
    ...
    print(f"\nFINAL Macro-F1 SCORE (W/o Jaccard): {macro_f1:.4f}")
